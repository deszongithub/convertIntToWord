...
public class WordMaker {
	...
	private String processDeHun(String s, boolean addAnd) {
		...
		switch (l.size()) {
			...
			{
				return addAnd
						? l.get(1) + DeUnit.AND.getVal() + l.get(0)
						: l.get(0) + l.get(1).substring(3);
			}
	...
	/**
	 * tail recursion using numbers sublist and converting each element adding
	 * to WordBuilder on each recursion.
	 */
	public Word buildWord(List<String> fmtNumber, WordBuilder wordBuilder,
			NumberWordMapping wordMapping) {
		var sz = fmtNumber.size();

		// get zero index element and convert
		...
		if (!num.isBlank() && !zero.toLowerCase().equals(num)) {
			...
			num = wordMapping.getId().equals(DE.name())
					? processDeHun(num, Integer.parseInt(firstElem) % 100 > 20)
					: num;

			switch (sz) {

				case 7 ->
					wordBuilder.quint(num + SPACE + wordMapping.getQuintn());
				case 6 ->
					wordBuilder.quadr(num + SPACE + wordMapping.getQuadrn());
				case 5 ->
					wordBuilder.trill(num + SPACE + wordMapping.getTrilln());
				case 4 ->
					wordBuilder.bill(num + SPACE + wordMapping.getBilln());
				case 3 ->
					wordBuilder.mill(num + SPACE + wordMapping.getMilln());
				case 2 ->
					wordBuilder.thou(num + SPACE + wordMapping.getThoud());
		...
		return sz == 1
				? wordBuilder.build()
				: buildWord(fmtNumber.subList(1, fmtNumber.size()), wordBuilder,
						wordMapping);
